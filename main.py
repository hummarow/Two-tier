    ...
    if not args.optuna:
        ...
        if utils.is_main_process():
            try:
                ...
                input("Warning: Output directory already exists and is not empty. Press Enter to continue...")
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            test_stats = evaluate(data_loader_val, model, criterion, device, writer, epoch, seed=args.seed+10000, ep=args.valepisodes)
...
class Objective:
    ...
    def __call__(self, trial):
        ...
        if 'inner_steps' in args.optuna:
            args.inner_steps = trial.suggest_int('inner_steps', 1, 4)
            print_param += f'inner_steps: {args.inner_steps}, '
        if 'inner_lr' in args.optuna:
            args.inner_lr = trial.suggest_float('inner_lr', 1e-2, 1e-1, log=True)
            print_param += f'inner_lr: {args.inner_lr}, '
        if 'contrastive_lr' in args.optuna:
            args.contrastive_lr = trial.suggest_float('contrastive_lr', 1e-5, 1e-1, log=True)
